...
def extractFemaleNames(filename,letter):
  ...
  f = open(filename,'r')
  txt = f.read()
  female_name = re.findall(r'<td>(\d+)</td><td>(\w+)</td><td>(\w+)</td>',txt)
  for i in female_name:
    if i[2][0] == letter:
      female_names.append(i[2]+ ' '+i[0])
...
def extractMaleNames(filename,letter):
  ...
  f = open(filename,'r')
  txt = f.read()
  male_name = re.findall(r'<td>(\d+)</td><td>(\w+)</td><td>(\w+)</td>',txt)
  for i in male_name:
    if i[1][0] == letter:
      Male_names.append(i[1]+' '+i[0])
...
def extractDocAndPopularityDate(filename):
  f = open(filename,'r')
  txt = f.read()
  header = re.findall(r'(\w+ \d+, \d+)</td>',txt)
  header1 = re.findall(r'(\w+ \w+ \d+)</h3>',txt)
  total = str(header[0])+" and " +str(header1[0])
